...
.available_filters_choices.tblist <- function(source, cohort, ...) {
  ...
  # `shape()` provides the filter name and a ready-made description (combining
  # the filter- and variable-level descriptions). Domains are not needed here
  # (we only render name + description), so skip computing them.
  filters_shape <- cohortBuilder::shape(source, domains = FALSE)$filters

  choices <- purrr::map(available_filters, function(x) {
    shape_entry <- filters_shape[[x@id]]
    tibble::tibble(
      name = shape_entry$name %||% x@name,
      id = x@id,
      dataset = x@dataset,
      description = shape_entry$description %||% NA_character_
    )
  }) |> dplyr::bind_rows()
