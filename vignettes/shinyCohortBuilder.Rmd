...
You can customize the input controller with `gui_args` argument (e.g. you disable search option for discrete filter (`shinyWidgets::virtualSelectInput`) with `gui_args = list(search = FALSE)`).
...
Argument `gui_args` can be also used to customize the controller.
