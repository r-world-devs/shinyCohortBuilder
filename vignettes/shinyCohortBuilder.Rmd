...
When having multiple options to choose, the approach may be become inconvenient.
...
5. Mapping for discrete filter options
...
The filters will be stored within a Source, so it's enought to pass such object to Cohort:
...
Providing custom rules will be added in the future released of `shinyCohortBuilder`.
...
There is a one assumption regarding `Add Step` feature - newly added filtering step is replicated based on the last available step in the Cohort (with choices and statistics related to the previous step).

In the future releases we plan to allow adding customized steps. 
Currently you can provide different steps in the panel only by defining them directly in the Cohort (before rendering GUI).
...
When the option is enabled a "play" button is displayed at each filtering step panel.
