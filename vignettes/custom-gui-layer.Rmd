...
- It's recommended the statistics are taken from the Cohort statistics `cohort$get_stats(step_id, state = "pre")`.
...
.update_data_stats.default <- function(source, step_id, cohort, session, ...) {
  ...
  session$output[[paste0(step_id, "-stats")]] <- shiny::renderUI({
    previous <- cohort$get_stats(step_id, state = "pre")$n_rows
    if (!previous > 0) {
...
.update_data_stats.tblist <- function(source, step_id, cohort, session, ...) {
  ...
  dataset_names %>% purrr::walk(
    ~ .sendOutput(
      ...
      shiny::renderUI({
        previous <- cohort$get_stats(step_id, state = "pre")[[.x]]$n_rows
        if (!previous > 0) {
          return("No data selected in previous step.")
        }
        current <- cohort$get_stats(step_id, state = "post")[[.x]]$n_rows
        .pre_post_stats(current, previous, percent = TRUE, stats = stats)
      }),
      session
    )
...
- When `attach_as = "meta"` attach filters to the `available_filters` Source element (`source$available_filters`).
