...
- It's recommended the output performs previous step data validation (in terms of data existance) and returns descriptive message to the user.
- Use `cohort$attributes$stats` to get displayed statstics state chosen by the user ("pre", "post", both or NULL).
- For printing the statistics use `.pre_post_stats` which returns formatted statistics output.
...
- Generate filters based on Source data (i.e. column types) using `cohortBuilder::filter` and wrap them with `cohortBuilder::step`.
- Attach the filters to the Source using `add_step` method.
- The method should return source with configured filters.
...
autofilter.tblist <- function(source, ...) {
  step_rule <- source$attributes$datasets %>%
...
4. **`.step_attrition` - define how step attrition plot should be rendered**
...
5. **`.custom_attrition` - (optional) a custom method used for your own version of attrition plot**
