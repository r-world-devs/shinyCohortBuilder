...
test_that("autofilter.tblist with attach_as='meta' stores as available_filters", {
  ...
  result <- autofilter(source, attach_as = "meta")

  expect_true(length(result$attributes$available_filters) >= 2)
})
...
test_that("autofilter.tblist creates discrete_text for unique character columns", {
  ...
  expect_true("discrete_text" %in% class(id_filter))
})
...
test_that("autofilter.tblist uses vs gui_input for >3 unique character values", {
  ...
  expect_true("discrete" %in% class(grp_filter))
  expect_equal(grp_filter$get_params("gui_input"), "vs")
})
...
test_that(".available_filters_choices.tblist returns prepared choices", {
  ...
  # available_filters need to be evaluated (bound to source) filter objects
  evaluated_filters <- purrr::map(
    source$attributes$available_filters,
    ~ .(source)
  )
  coh$attributes$available_filters <- evaluated_filters
